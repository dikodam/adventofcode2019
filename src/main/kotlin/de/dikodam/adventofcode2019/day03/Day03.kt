...
fun main() {
    ...
    val (wireData, setupDuration) = withTimer {
        val wireCoordinates = day03input
            .map { it.split(",") }
            .map(List<String>::parseInstructions)
            .map(List<Day03PathInstruction>::toCoordinates)
        val (wire1, wire2) = wireCoordinates
        val intersections = wire1.intersect(wire2)
        Triple(wire1, wire2, intersections)
    }

    val (wire1, wire2, intersections) = wireData

    val (distanceTask1, t1duration) = withTimer {
        intersections
            .map { (x, y) -> manhattanDistance(x, y) }
            .sorted()[0]
    }
...
data class Day03PathInstruction(val direction: Char, val count: Int)
data class Day03Coordinates(val x: Int, val y: Int)

fun List<String>.parseInstructions() = this.map { Day03PathInstruction(it[0], it.substring(1).toInt()) }

fun List<Day03PathInstruction>.toCoordinates(): List<Day03Coordinates> {
    var current = Day03Coordinates(0, 0)
    val accumulator = mutableListOf<Day03Coordinates>()

    for (instruction in this) {
        repeat(instruction.count) {
            current = when (instruction.direction) {
                'R' -> Day03Coordinates(current.x + 1, current.y)
                'L' -> Day03Coordinates(current.x - 1, current.y)
                'U' -> Day03Coordinates(current.x, current.y + 1)
                'D' -> Day03Coordinates(current.x, current.y - 1)
                else -> throw IllegalArgumentException("Cannot process instruction $instruction")
            }
            accumulator += current
        }
    }
...
fun stepsNeededToReach(target: Day03Coordinates, wire: List<Day03Coordinates>) = wire.indexOf(target) + 1
